...
- **REQ-scan-001** (MODIFIED): Camera initialization with facingMode fallback
  - Description: The app requests camera permission using `navigator.mediaDevices.getUserMedia` on first access. It first attempts `{ video: { facingMode: 'environment' } }` to prefer the rear camera. If `OverconstrainedError` is thrown (no rear camera available), it falls back silently to `{ video: true }` (any available camera). A live camera viewfinder is displayed once permission is granted. If the camera API is unavailable (`getUserMedia` not supported), a fallback message is shown explaining the requirement.
  - Priority: P0
...
- **REQ-scan-002** (MODIFIED): Camera error handling with distinct messages
  - Description: If the camera stream cannot be obtained, the app checks the error type and shows a distinct, actionable error message:
    ...
    A manual barcode entry field is shown alongside every error state so staff can continue working even if the camera fails entirely.
  - Priority: P0
...
- **REQ-scan-004** (MODIFIED): Decode failure handling with network resilience
  - Description: The `@zxing/library` dynamic import includes a `.catch()` handler. If the polyfill script fails to load (slow network, CDN down, script blocked), the app shows a manual entry form instead of crashing. When the camera stream fails to decode a barcode after a reasonable attempt (poor lighting, damaged label, motion blur), a non-blocking retry prompt is shown with the viewfinder remaining active. Manual barcode entry is always available as an alternative.
  - Priority: P0
...
- **REQ-scan-007** (MODIFIED): Continuous scan support
  - Description: After a successful scan and result display, the scan state resets automatically so the next barcode can be decoded without navigating away. Previously `scannedRef.current` remained set after the first scan, locking the decoder. The camera stream remains active across scans without re-requesting permission. The app supports an unlimited sequence of scan -> result -> scan cycles.
  - Priority: P0
...
- **REQ-scan-009** (ADDED): Video readyState timeout
  - Description: A 10-second timeout is added to the polling loop that waits for `video.readyState >= 2` (video data available). If the readyState does not reach `HAVE_CURRENT_DATA` within 10 seconds, the polling stops and a timeout message is shown: "Camera stream is taking too long to start. Try reloading the page or use manual entry below." The timeout value is defined as a tunable constant.
  - Priority: P0
...
- **REQ-scan-010** (ADDED): Camera permission pre-check
  - Description: Before calling `getUserMedia`, the app checks `navigator.permissions.query({ name: 'camera' })` where the API is supported (feature-detected). If the permission state is `denied`, the app shows the `NotAllowedError` message without making a `getUserMedia` call. If the permission API is unsupported (Safari, older browsers), the check is skipped silently and `getUserMedia` is called directly.
  - Priority: P1
...
- **S-scan-007** (ADDED): Staff on device with front camera only (iPad)
...
- **S-scan-008** (ADDED): Staff opens scan on HTTP (not HTTPS)
...
- **S-scan-009** (ADDED): Staff scans multiple products in succession
...
- **S-scan-010** (ADDED): Staff on slow network, polyfill fails to load
